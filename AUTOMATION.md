...
- 各記事の要約に続けて「影響が考えられる業種」と「注目点・リスク」を必ず短く添える。前者は関連が強い業種を1〜3個、後者は影響の経路と今後確認すべき点・不確実性を1〜2文で記す。
- 業種への影響は確認済み事実に基づく分析として示し、事実と推論を区別する。根拠のない銘柄推奨や株価の上昇・下落の断定をしない。関連が不明確な場合はその旨を記し、無理に業種を挙げない。
- 上記2項目は、既存の各 `<article class="topic">` 内で要約の後に、それぞれラベル付きの `<p>` として追加する。既発行号の本文は遡って変更しない。
...
- 全記事に「影響が考えられる業種」と「注目点・リスク」があり、短く具体的で、確認済み事実と分析・不確実性が区別されている。
